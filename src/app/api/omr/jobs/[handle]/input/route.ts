...
import { authorizeOmr, omrHandle, parseVendorInputBody, readBoundedJson } from "../../../../../../server/http/omr-api";
...
export async function POST(request: NextRequest, context: { readonly params: Promise<{ readonly handle: string }> }): Promise<NextResponse> {
  try {
    ...
    await application.submitInput(omrHandle(handle), parseVendorInputBody(await readBoundedJson(request)));
