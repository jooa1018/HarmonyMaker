...
export async function mapVendorEvidenceToSource(input: { readonly sourceRevision: SourceRevisionRef; readonly mappingVersion: string; readonly vendorBundle: VendorEvidenceBundle; readonly targetMappings: readonly EvidenceTargetMapping[] }): Promise<{ readonly index: SourceEvidenceIndex; readonly archive: OmrEvidenceArchive }> {
  ...
  const withoutId = (item: OmrEvidence): object => Object.fromEntries(Object.entries(item).filter(([key]) => key !== "id"));
  const bundleDigest = await semanticDigest({ projectionSchema: "hm-source-evidence-index-v1", ...common, mappingVersion: input.mappingVersion, evidence: evidence.map(withoutId), targetMappings: input.targetMappings });
  const archiveDigest = await semanticDigest({ projectionSchema: "hm-omr-evidence-archive-v1", ...common, unmappedEvidence: unmappedEvidence.map(withoutId) });
