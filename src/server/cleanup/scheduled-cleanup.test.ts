...
describe("production scheduled cleanup entrypoint", () => {
  ...
  it("invokes generic and provider-aware OMR cleanup in one bounded structured run", async () => {
    ...
    const omr = { cleanupExpiredJobs: vi.fn(async () => ({ attemptedJobs: 1, completedJobs: 1, pendingJobs: 0, failedJobs: 0 })) };
    ...
    expect(result).toMatchObject({ ok: false, batchSize: 25, generic: { status: "fulfilled", expiredSessions: 1, attemptedItems: 1, completedItems: 0, failedItems: 1 }, omr: { status: "fulfilled", attemptedJobs: 1, completedJobs: 1, pendingJobs: 0, failedJobs: 0 } });
  ...
  it("returns before a never-resolving domain exceeds the enforced deadline", async () => {
    ...
    const run = runScheduledCleanup({
      ...
      omr: { cleanupExpiredJobs: async () => ({ attemptedJobs: 0, completedJobs: 0, pendingJobs: 0, failedJobs: 0 }) },
      runtimeBudgetMs: 25,
  ...
  it("isolates domain failures and accepts concurrent bounded invocations", async () => {
    ...
    const omr = { cleanupExpiredJobs: vi.fn(async () => ({ attemptedJobs: 0, completedJobs: 0, pendingJobs: 0, failedJobs: 0 })) };
  ...
  it("surfaces isolated OMR item failures as a partial scheduler result", async () => {
    ...
    const result = await runScheduledCleanup({
      ...
      omr: { cleanupExpiredJobs: async () => ({ attemptedJobs: 3, completedJobs: 2, pendingJobs: 0, failedJobs: 1 }) },
      now: () => new Date("2026-01-01T00:00:00.000Z"),
    });
    expect(result).toMatchObject({ ok: false, omr: { status: "fulfilled", attemptedJobs: 3, completedJobs: 2, pendingJobs: 0, failedJobs: 1 } });
    expect(scheduledCleanupHttpStatus(result)).toBe(207);
  });

  it("reports retryable OMR cleanup as pending rather than completed and returns 207", async () => {
    vi.spyOn(console, "info").mockImplementation(() => undefined);
    const result = await runScheduledCleanup({
      generic: { run: async () => emptyGeneric } as never,
      omr: { cleanupExpiredJobs: async () => ({ attemptedJobs: 1, completedJobs: 0, pendingJobs: 1, failedJobs: 0 }) },
      now: () => new Date("2026-01-01T00:00:00.000Z"),
    });
    expect(result).toMatchObject({ ok: false, omr: { status: "fulfilled", attemptedJobs: 1, completedJobs: 0, pendingJobs: 1, failedJobs: 0 } });
    expect(scheduledCleanupHttpStatus(result)).toBe(207);
  });
  ...
  it("returns an exact successful 200-ready result when both domains complete without item failures", async () => {
    ...
    const result = await runScheduledCleanup({
      generic: { run: async () => emptyGeneric } as never,
      omr: { cleanupExpiredJobs: async () => ({ attemptedJobs: 2, completedJobs: 2, pendingJobs: 0, failedJobs: 0 }) },
      now: () => new Date("2026-01-01T00:00:00.000Z"),
    });
    expect(result).toMatchObject({
      ok: true,
      generic: { status: "fulfilled", attemptedItems: 0, completedItems: 0, failedItems: 0 },
      omr: { status: "fulfilled", attemptedJobs: 2, completedJobs: 2, pendingJobs: 0, failedJobs: 0 },
    });
  ...
  it("rejects attempts to expand one invocation beyond the fixed limits", async () => {
    await expect(runScheduledCleanup({ generic: { run: async () => emptyGeneric } as never, omr: { cleanupExpiredJobs: async () => ({ attemptedJobs: 0, completedJobs: 0, pendingJobs: 0, failedJobs: 0 }) }, batchSize: 51 })).rejects.toThrow("CLEANUP_BATCH_INVALID");
    await expect(runScheduledCleanup({ generic: { run: async () => emptyGeneric } as never, omr: { cleanupExpiredJobs: async () => ({ attemptedJobs: 0, completedJobs: 0, pendingJobs: 0, failedJobs: 0 }) }, runtimeBudgetMs: 25_001 })).rejects.toThrow("CLEANUP_RUNTIME_BUDGET_INVALID");
  });
});
