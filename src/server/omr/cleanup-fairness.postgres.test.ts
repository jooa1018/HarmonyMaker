...
async function session(): Promise<PrivateRowId> {
  const result = await pool.query(
    "INSERT INTO anonymous_sessions (token_hash,csrf_nonce,created_at,expires_at) VALUES ($1,$2,$3,$4) RETURNING id",
    ["token:cleanup-fairness", "csrf:cleanup-fairness", "2025-12-01T00:00:00.000Z", "2027-01-01T00:00:00.000Z"],
  );
...
describe("actual PostgreSQL OMR cleanup fairness", () => {
  it("does not let 25 persistent oldest rows starve every later due row", async () => {
    const store = new PostgresOmrStore(pool);
    const ownerSessionId = await session();
    const allIds: PrivateRowId[] = [];
    ...
    const rows = await pool.query("SELECT count(*)::int AS count FROM omr_jobs WHERE cleanup_last_attempt_at IS NOT NULL");
    expect(Number(rows.rows[0].count)).toBe(50);
  });
});
